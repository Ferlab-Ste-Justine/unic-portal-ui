...
describe('Tableau Tables - Valider les liens du filtre Resource', () => {
  it('Reset filters - Clear Input', () => {
    ...
    cy.get('[id*="TablesTable"] [class*="InputSelect_filter"] [title="LVC-Bronchiolite-HSJ"]').should('not.exist');
  });

  it('Reset filters - Results', () => {
    cy.get('[id*="TablesTable"] [class*="Header_clearFilterLink"]').clickAndWait();
    cy.get('[id*="TablesTable"] [class*="Header_ProTableHeader"]').contains(/(^13 Results$| of 13$)/).should('not.exist');
  });
